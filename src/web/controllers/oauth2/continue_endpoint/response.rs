...
use crate::{
    application::error::{
        AppError,
        codes::authorize::AuthorizeErrorCode,
    },
    controllers::response::redirect_to_response,
    domain::openid_connect::{AuthorizationRequestData, OAuthErrorCode, OAuthErrorResponse, ResponseType},
};
...
pub(super) fn continue_oauth_error_response(
    ...
    let response_type = request.response_type.parse::<ResponseType>().map_err(|error| {
    ...
    Ok(match response_mode_from_value(request.response_mode.as_deref()) {
        ...
        _ if response_type.uses_front_channel_response() => {
            redirect_to_response(error_response.to_fragment_redirect_url(&redirect_uri).as_str())
        }
